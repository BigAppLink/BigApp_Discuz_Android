...
public class AutoScrollTextView extends TextView {
	...
	private Runnable ScrollRunnable = new Runnable() {
		...
		public void run() {
			...
			} else if (autoScrollY > maxY) {
				isAdd = false;
			}
			if (isAdd) {
				autoScrollY = autoScrollY + speed;
			} else {
				autoScrollY = autoScrollY - speed;
			}

			if (autoScroll == true && maxY > 0) {
				// 如果是自动滚动并且搞过空间高度
				scrollTo(0, autoScrollY);
				mHandler.postDelayed(this, howLong);
			} else {
	...
	private boolean processScroll(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (mIsBeingDragged && INVALID_POINTER != pointerId) {
				...
				velocityTracker.computeCurrentVelocity(howLong,
						mMaximumVelocity);
