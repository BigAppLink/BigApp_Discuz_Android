...
public class HttpCache {
    ...
    public static final int CACHE_AND_REFRESH = 3;
    ...
    public boolean needRefresh() {
    	return cacheMode != HttpCache.DEFAULT;
    }
    
    
    public boolean isEnabled(HttpRequest.HttpMethod method) {
        if (method == null || (cacheMode == ONLY_NET)) return false;
    ...
    public boolean isEnabled(String method) {
        if (TextUtils.isEmpty(method) || (cacheMode == ONLY_NET)) return false;
    ...
    static {
        ...
        httpMethod_enabled_map.put(HttpRequest.HttpMethod.POST.toString(), true);
    }
}
